...
def generate_uniform_static_points(width: int, height: int, count: int) -> List[Point]:
    ...
    The layout uses a coarse lattice so fleet-size sweeps continue to add
    spatial value instead of duplicating the same loiter locations.
    """
    if count <= 0:
        return []
...
def resolve_static_points(
    ...
    Modes:
    - explicit: honor provided points, but append unique lattice points if the
      fleet is larger than the explicit list.
